...
void Keygeneration(element_t xi, element_t Xi,  element_t P){
    element_random(xi);
          ...
          pairing_t pairing)
{
    ...
    element_t h2, h3, h5, h6, temp_zr, temp_z2;
    element_t temp_g1, temp_g2, temp_g3, temp_g4, temp_g5;
...
int aggregate_verify(
    ...
    element_t alpha, beta, h2t, h5t, h6t, h1i, h3i, temp_Zr;
    ...
    element_add(alpha, P_agg, H_agg);//P_gg+H_gg
       ...
       pairing_apply(e2, Y, alpha, pairing); //e(Q, P_gg+H_gg)
        ...
        pairing_apply(e4, G_agg, h6t, pairing);
...
{   Vehicle V[40];
  ...
  elapsed12 = ((end12.tv_sec - start12.tv_sec) + (end12.tv_nsec - start12.tv_nsec) / 1e9)/40;  
    ...
    printf("execution times of 0-40 signing algorithms is:\n");
      clock_gettime(CLOCK_MONOTONIC, &start2);
    for(int i=0; i<40; i++){
       sign(si[i], ti[i], V[i].IDi, V[i].SP1, V[i].SP2, V[i].xi, P, msgs[i], t, pairing);
  ...
  printf("execution times of an agg-verificaiton algorithm of 2-40 signatures:\n");
    ...
    for(int q=2; q<=40; q++){
      ...
      int result=aggregate_verify(pairing, V, S, T, msgs,   t, P, Y, q);
      if(result){
