...
void Keygeneration(element_t xi, element_t Xi,  element_t zi, element_t Zi, element_t P){
    element_random(xi);
    element_mul_zn(Xi, P, xi);
    element_random(zi);
    element_mul_zn(Zi, P, zi);
          ...
          pairing_t pairing)
{
    ...
    element_add(temp_z2, temp_z2, zi);
...
int aggregate_verify(
    ...
    for (int i = 0; i < q; i++) {
      ...
      H4(P2[i], IDi, "2");//Pi2
        ...
        element_add(P4[i], P4[i], vehicles[i].Zi);
...
{   Vehicle V[40];
    ...
    sha1_binary(t, hash_bin);
    char hex_hash[41];  
    element_t P, Q, B, Y, y, h1i, h2i, h3i, si[40], ti[40], left, right, S, T;
    element_init_Zr(h1i, pairing);
    ...
    element_init_GT(right, pairing);
     ...
     for(int i=0; i<40; i++){
         ...
         element_init_Zr(V[i].zi, pairing);
         element_init_G1(V[i].Zi, pairing);
    }
  ...
  elapsed12 = ((end12.tv_sec - start12.tv_sec) + (end12.tv_nsec - start12.tv_nsec) / 1e9)/40;  
      ...
      for(int i=0; i<40; i++){
       Keygeneration(V[i].xi, V[i].Xi, V[i].zi, V[i].Zi, P);
    }
    ...
    for(int i=0; i<40; i++){
       sign(si[i], ti[i], V[i].IDi, V[i].SP1, V[i].SP2, V[i].xi, V[i].zi, P, msgs[i], t, pairing);
